...
namespace WhaleLand.Extensions.DynamicRoute.Consul
{
    class ConsulServiceDiscoveryProvider : IServiceDiscoveryProvider
    {
        ...
        {
            var policy = Polly.Policy.Handle<Exception>().WaitAndRetryForever((int a) => TimeSpan.FromSeconds(5.0), delegate (Exception ex, TimeSpan time)
            {
        ...
        {
            var policy = Polly.Policy.Handle<Exception>().Or<IOException>().WaitAndRetryForever((int a) => TimeSpan.FromSeconds(5.0), delegate (Exception ex, TimeSpan time)
            {
                _logger.LogError(ex, ex.Message, Array.Empty<object>());
            });
