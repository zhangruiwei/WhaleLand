            ...
            try
            {
                ...
                for (var eventIndex = 0; eventIndex < Events.Count; eventIndex++)
                {
                    ...
                    await _senderRetryPolicy.ExecuteAsync((ct) =>
                    {
                        ...
                        Memory<byte> bytes = Encoding.UTF8.GetBytes(json).AsMemory();
            ...
            for (int i = 0; i < _reveiverMaxDegreeOfParallelism; i++)
            {
                System.Threading.Tasks.Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Received"))
                            {
                                try
                                {
                                    ...
                                    var eventResponse = new EventResponse()
                                    {
                                        ...
                                        QueueName = queueName,
                                        RouteKey = routeKey,
                                        BodySource = Encoding.UTF8.GetString(ea.Body.ToArray()),
                                        Body = default(TD),
                                    };
                    ...
                    Task.Run(async () =>
                    {
                        while (true)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    using (var receiveTracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Received"))
                                    {
                                        ...
                                        try
                                        {
                                            for (int i = 0; i < basicGetResults.Length; i++)
                                            {
                                                ...
                                                using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP BasicGet"))
                                                {
                                                    tracer.SetComponent(_compomentName);
                                                    tracer.SetTag("queueName", queueName);
                                                    tracer.SetTag("x-messageId", MessageId);
