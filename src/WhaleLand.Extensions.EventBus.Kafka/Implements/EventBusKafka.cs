...
namespace WhaleLand.Extensions.EventBus.Kafka
{
    ...
    public class EventBusKafka : IEventBus
    {
        ...
        public async Task PublishNonConfirmAsync(List<Models.EventLogEntry> Events, CancellationToken cancellationToken)
        {
            ...
            using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP"))
            {
        ...
        private void Enqueue(List<EventMessage> Events, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (var topic in groups)
                {
                    ...
                    {
                        using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Publish"))
                        {
                            tracer.SetComponent(_compomentName);
                ...
                try
                {
                    ...
                    while (!cancellationToken.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            {
                                _logger.LogInformation($"Consumed message '{ea.Message.Value}' at: '{ea.TopicPartitionOffset}'.");
                            }

                            var EventId = -1L;
                            var MessageId = ea.Message.Key;
                            var TraceId = MessageId;

                            using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Received", TraceId))
                            {
                                #region 获取EventId 和 TracerId
                                if (ea.Message.Headers != null)
                                {
                                    try
                                    {
                                        long.TryParse(System.Text.Encoding.UTF8.GetString(ea.Message.Headers.GetLastBytes("x-eventId")), out EventId);
                                    }
                                    catch
                                    { }
                                    ...
                                    try
                                    {
                                        TraceId = System.Text.Encoding.UTF8.GetString(ea.Message.Headers.GetLastBytes("x-traceId"));
                                    }
                                    catch
                                    {
                                    }
                                ...
                                try
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        #region 设置header
                                        foreach (var key in ea.Message.Headers)
                                        {
                                    ...
                                    #region 处理消息
                                    using (var tracerExecuteAsync = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Execute"))
                                    {
                            ...
                            try
                            {
                                ...
                                for (int j = 0; j < eas.Length; j++)
                                {
                                    ...
                                    using (var tracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Received", TraceId))
                                    {
                                        ...
                                        if (ea.Message.Headers != null && ea.Message.Headers.Count > 0)
                                        {
                                            try
                                            {
                                                long.TryParse(System.Text.Encoding.UTF8.GetString(ea.Message.Headers.GetLastBytes("x-eventId")), out EventId);
                                ...
                                {
                                    using (var executeTracer = new WhaleLand.Extensions.OpenTracing.Tracer("AMQP Execute"))
                                    {
